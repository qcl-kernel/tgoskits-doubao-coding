...
| [#2175](https://github.com/rcore-os/tgoskits/pull/2175) | `feat(axvisor): add AArch64 and RK3588 AMP support` | 增加编译期实时核选择和实时任务创建入口，完成 AArch64 Starry guest 与宿主实时任务共存，并补充 RK3588/OrangePi 5 Plus AMP 构建运行配置 | `components/axsched/`、`os/arceos/modules/axtask/`、`os/axvisor/`、`virtualization/axvm/`、`scripts/axbuild/`、`test-suit/axvisor/` | `axvm` 单元测试和目标 crate Clippy；AArch64 QEMU host-only、Starry + host AMP 联合用例；OrangePi 5 Plus 板级构建入口 | PR open，base `dev` |
