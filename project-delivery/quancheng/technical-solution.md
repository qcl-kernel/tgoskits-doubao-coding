...
该算法对实时性的要求来自平衡控制本身，而不是通信协议。语音识别可能需要数百毫秒，属于上层决策输入；一旦命令到达实时侧，机器人保持平衡仍依赖 8ms 闭环持续运行。若智能侧 AI 推理、文件系统、网络或日志负载抢占控制侧 CPU，会直接放大控制周期抖动。因此任务一中的实时 CPU 预留、RT FIFO 调度和 mutex 优先级继承，是让该实物演示稳定运行的必要底座。
